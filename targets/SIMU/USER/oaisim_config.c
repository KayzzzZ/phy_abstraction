...
void oaisim_config(void)
{
  ...
  if (oai_emulation.info.ocg_enabled == 1) { // activate OCG: xml-based scenario parser   ------ default ocg_enabled = 0, unless type '-c'
...
#endif
  olg_config();      //log gen, set log_level and log_verbosity
  ocg_config_emu();    //
  ocg_config_env();// mobility gen
  ocg_config_topo(); // packet tracer using wireshark
  ocg_config_proto();  //just log

  // if T is set or ocg enabled
  if (oai_emulation.info.otg_enabled ) {  //in init, set to 0
...
int olg_config(void)
{
  ...
  // fix me:
  oai_emulation.info.g_log_level     = ((oai_emulation.info.ocg_enabled == 1)  && (ocg_log_level != -1))     ? ocg_log_level : oai_emulation.info.g_log_level;  //set log_level
  oai_emulation.info.g_log_verbosity = (((oai_emulation.info.ocg_enabled == 1) && (ocg_log_verbosity != -1)) ? ocg_log_verbosity :  //set log_verbosity
  ...
  */
#if defined(ENABLE_USE_MME)  //not defined
...
int ocg_config_env(void)
{
  ...
  if (oai_emulation.info.ocg_enabled) {  // in init, set to 0
    for (CC_id=0; CC_id<MAX_NUM_CCs; CC_id++) {
...
int ocg_config_emu(void)
{
  ...
  if (oai_emulation.info.n_frames_flag) { //default is 0
  ...
  if (oai_emulation.info.cli_enabled == 1) {  //in init, set to 0
  ...
  if (oai_emulation.info.opp_enabled)  //in init, set to 0
    opp_enabled = 1;

  //bin/LOG_I(OCG, "OPT output file directory = %s\n", oai_emulation.info.output_path);
  // in init, packet_trace.enabled was set to 0, as a result, opt_enabled was 0.
  ...
  if (oai_emulation.info.opt_enabled == 1) {  //0
    radio_type_t radio_type;
    opt_enabled = 1;

    if (oai_emulation.info.frame_type[0] == FDD) {
      radio_type = RADIO_TYPE_FDD;   //  1
    } else {
      radio_type = RADIO_TYPE_TDD;
    }

    opt_type = oai_emulation.info.opt_mode;  // 1
