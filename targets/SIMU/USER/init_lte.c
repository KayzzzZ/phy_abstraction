                           ...
                           uint8_t abstraction_flag)
{
  ...
  int i,j;
  PHY_VARS_eNB* PHY_vars_eNB = malloc(sizeof(PHY_VARS_eNB));  //malloc doesn't clear the memory, so we need memset()
		   ...
		   uint8_t eMBMS_active_state)
{
  ...
  for (eNB_id=0; eNB_id<NB_eNB_INST; eNB_id++) {  //NB_eNB_INST = 1
  ...
  for (UE_id=0; UE_id<NB_UE_INST; UE_id++) {  //NB_UE_INST = 1
