...
void check_and_adjust_params(void)
{
  ...
  if (oai_emulation.info.nb_ue_local  + oai_emulation.info.nb_rn_local > NUMBER_OF_UE_MAX) {  //nb_ue_local=1, nb_rn_local=0
    LOG_E(EMU,"Enter fewer than %d UEs/RNs for the moment or change the NUMBER_OF_UE_MAX\n", NUMBER_OF_UE_MAX);
    exit(EXIT_FAILURE);
  }

  if (oai_emulation.info.nb_enb_local + oai_emulation.info.nb_rn_local > NUMBER_OF_eNB_MAX) {  //nb_enb_local=1, nb_rn_local=0
    LOG_E(EMU,"Enter fewer than %d eNBs/RNs for the moment or change the NUMBER_OF_UE_MAX\n", NUMBER_OF_eNB_MAX);
    exit(EXIT_FAILURE);
  }

  if (oai_emulation.info.nb_rn_local > NUMBER_OF_RN_MAX) {  //nb_rn_local=0
...
#ifdef RRC_CELLULAR  //not defined
  abstraction_flag = 1;
  ethernet_flag = 1;
#endif

  if (set_sinr == 0)   //set_sinr = 0
    sinr_dB = snr_dB - 20; // 30-20
  ...
  if (ethernet_flag == 1) {  //ethernet_flag=0, skip
  ...
  //
  NB_UE_INST = oai_emulation.info.nb_ue_local + oai_emulation.info.nb_ue_remote; //remote = 0
  NB_eNB_INST = oai_emulation.info.nb_enb_local + oai_emulation.info.nb_enb_remote;  // remote = 0
  NB_RN_INST = oai_emulation.info.nb_rn_local + oai_emulation.info.nb_rn_remote;  // remote = 0

#if defined(PDCP_USE_NETLINK_QUEUES) && defined(OPENAIR2)  //not defined PDCP_USE_NETLINK_QUEUES, skip
  pdcp_netlink_init();
#endif

  if (NB_RN_INST > 0 ) { //NB_RN_INST=0, skip
...
void init_omv(void)
{
  if (oai_emulation.info.omv_enabled == 1) { //0, skip
  ...
  for (UE_id=0; UE_id<NB_UE_INST; UE_id++)
    for (CC_id=0; CC_id<MAX_NUM_CCs; CC_id++) {
      ...
      if (abstraction_flag == 0)
        ...
        PHY_vars_UE_g[UE_id][CC_id]->UE_mode[0] = PRACH;  //skip the sync process, begin with prach
      }
...
void init_otg_pdcp_buffer(void) //  otg = openair traffic gen
{
  module_id_t i;
...
{
  if (oai_emulation.info.slot_isr) {  // in init, set to 0, skip
...
void init_time()
{
  ...
  td_avg        = 0;
  sleep_time_us = SLEEP_STEP_US;  //100,   0.01ms
  td_avg        = TARGET_SF_TIME_NS;  // 10e5,   1ms
}
