...
#define PRINT_STATS
  ...
  for (frame = 0;
    ...
    if (oai_emulation.info.oeh_enabled == 1)  //0, skip
      execute_events (frame);

    if (ue_connection_test == 1) {  // 0, skip
      if ((frame % 20) == 0) {
    ...
    if ((oai_emulation.info.omv_enabled == 1)) {  //0
    ...
    for (slot = 0; slot < 20; slot++) {
      ...
      direction = subframe_select (frame_parms[0], next_slot >> 1); //always SF_DL in FDD mode

#ifdef PROC  //not defined
...
#endif
      {
        ...
        for (eNB_inst = oai_emulation.info.first_enb_local; //oai_emulation.info.first_enb_local = 0
             (eNB_inst
              < (oai_emulation.info.first_enb_local
                 + oai_emulation.info.nb_enb_local));
             eNB_inst++) {
          if (oai_emulation.info.cli_start_enb[eNB_inst] != 0) {  // =1
            if ((slot & 1) == 0) {
...
#ifdef OPENAIR2
                  //Application
                  update_otg_UE (UE_inst, oai_emulation.info.time_ms);  //seems otg_enabled = 0, skip??
        ...
        emu_transport (frame % MAX_FRAME_NUMBER, last_slot, next_slot, direction,
                       oai_emulation.info.frame_type[0], ethernet_flag);   // ethernet_flag = 0, do nothing, skip
...
//#warning figure out what to do with UE frame_parms during initial_sync
        ...
        if ((direction == SF_UL) || (frame_parms[0]->frame_type == 0)) { //if ((subframe<2) || (subframe>4))  ,  frame_type = 0 : FDD
...
main (int argc, char **argv)
{
  ...
  check_and_adjust_params ();  //count the number of enb and ue

  set_seed = oai_emulation.emulation_config.seed.value;  //oai_emulation.emulation_config.seed.value=0

  init_otg_pdcp_buffer ();

  init_seed (set_seed);  //set_seed = 0

  init_openair1 ();

  init_openair2 ();

  init_ocm ();  //AWGN channel,  to be figure out!!!! 

#ifdef SMBV  //not defined
  // Rohde&Schwarz SMBV100A vector signal generator
  ...
  if (oai_emulation.info.oeh_enabled == 1)  // 0
    schedule_events ();

  // oai performance profiler is enabled
  if (oai_emulation.info.opp_enabled == 1)  // 0
    reset_opp_meas ();

  init_time ();  //

  init_slot_isr ();  //no process
  ...
  if (create_tasks(oai_emulation.info.nb_enb_local, oai_emulation.info.nb_ue_local) >= 0) {  // 4 tasks were created: l2l1_task  eNB_app_task  rrc_enb_task  rrc_ue_task
    itti_wait_tasks_end();
  } else {
