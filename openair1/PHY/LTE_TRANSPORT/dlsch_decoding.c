...
int dlsch_abstraction_MIESM(double* sinr_dB,uint8_t TM, uint32_t rb_alloc[4], uint8_t mcs,uint8_t dl_power_off)
{
  ...
  if (uniformrandom() < bler) { //uniformrandom() >= bler ----> decoding successful
                             ...
                             uint8_t eNB_id)
{
  ...
  switch (dlsch_id) {
  case 0: // SI
    dlsch_ue = phy_vars_ue->dlsch_ue_SI[eNB_id];  //address
    dlsch_eNB = PHY_vars_eNB_g[eNB_id2][CC_id]->dlsch_eNB_SI;  //fetch data from dlsch_eNB_SI , and memcpy to ue->harq_processes
  ...
  case 2: // TB0
    ...
    if (dlsch_abstraction_MIESM(phy_vars_ue->sinr_dB,
      ...
      // retransmission, never happened in our cases(Kay revised)
