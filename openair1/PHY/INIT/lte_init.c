                     ...
                     unsigned char abstraction_flag)
{
  ...
  for (eNB_id=0; eNB_id<3; eNB_id++) {  // for each eNB, in our cases, just one.

    if (abstraction_flag==0) {  //ab_flag = 1, skip to 1380
  ...
  for (UE_id=0; UE_id<NUMBER_OF_UE_MAX; UE_id++) {
    ...
    if (abstraction_flag==0) {  //  ab_flag = 1, skip to 1489
  ...
  if (abstraction_flag==0) {  //ab_flag = 1, skip to 1524
    if (is_secondary_eNB) {
      for (eNB_id=0; eNB_id<3; eNB_id++) {
