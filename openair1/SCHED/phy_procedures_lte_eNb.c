                           ...
                           relaying_type_t r_type,PHY_VARS_RN *phy_vars_rn)
{
...
#endif
  int frame = phy_vars_eNB->proc[sched_subframe].frame_tx;  //if subframe = n, then subframe_tx = n+1, subframe_rx = n-1, (ps: subframe = slot>>1)
  ...
  if (is_pmch_subframe(phy_vars_eNB->proc[sched_subframe].frame_tx,subframe,&phy_vars_eNB->lte_frame_parms)) {
    ...
    if (abstraction_flag==0) {  // ab_flag = 1, skip
  ...
  if (subframe == 0) {
    ...
    if (abstraction_flag==0) {
      ...
      generate_pbch_emul(phy_vars_eNB,pbch_pdu);  //in DOCS
    }
  ...
  ul_subframe = pdcch_alloc2ul_subframe(&phy_vars_eNB->lte_frame_parms,subframe);  //(subframe+4)%10
  ul_frame = pdcch_alloc2ul_frame(&phy_vars_eNB->lte_frame_parms,phy_vars_eNB->proc[sched_subframe].frame_tx,subframe);  //(frame_tx+(subframe>=6 ? 1 : 0))

  if ((subframe_select(&phy_vars_eNB->lte_frame_parms,ul_subframe)==SF_UL) ||
      (phy_vars_eNB->lte_frame_parms.frame_type == FDD)) {
    harq_pid = subframe2harq_pid(&phy_vars_eNB->lte_frame_parms,ul_frame,ul_subframe);  //return(((frame<<1)+subframe)&7);
                           ...
                           uint8_t SR_payload)
{
  ...
  // handle case where positive SR was transmitted with multiplexing   !!!pucch_sel is TDD SPEC, in FDD, always set to 0
                      ...
                      int16_t *n1_pucch3)
{
  ...
  if (frame_parms->frame_type == FDD ) {
    ...
    if (phy_vars_eNB->dlsch_eNB[(uint32_t)UE_id][0]->subframe_tx[sf]>0) {  // will be set to 1 in eNB_TX  when eNB generate dlsch parameters from DCI and will be set to 0 before transmit new data
  ...
  if ((phy_vars_eNB->dlsch_eNB[UE_id][0]) &&
	...
	default:
	  ...
	  break;
        ...
        if ((n1_pucch0==-1) && (n1_pucch1==-1)) { // just check for SR, no ACK/NAK
...
void phy_procedures_eNB_RX(const unsigned char sched_subframe,PHY_VARS_eNB *phy_vars_eNB,const uint8_t abstraction_flag,const relaying_type_t r_type)
{
  ...
  if (abstraction_flag == 0) {  //ab_flag = 1, skip to 2739
  ...
  for (i=0; i<NUMBER_OF_UE_MAX; i++) {
	...
	process_Msg3(phy_vars_eNB,sched_subframe,i,harq_pid);  //set some flags
      }
    }
	...
	VCD_SIGNAL_DUMPER_DUMP_FUNCTION_BY_NAME(VCD_SIGNAL_DUMPER_FUNCTIONS_PHY_ENB_ULSCH_MSG3,0);
      ...
      if (ret == (1+MAX_TURBO_ITERATIONS)) {  //Kay revised, in our cases, ret always be set to 1, skip.
...
#   if ENABLE_RAL  //not defined, skip
...
#   endif
  ...
  for (CC_id=0; CC_id<MAX_NUM_CCs; CC_id++) {
    ...
    if (subframe_select(&phy_vars_eNB[CC_id]->lte_frame_parms,phy_vars_eNB[CC_id]->proc[subframe].subframe_tx)==SF_S) {  //subframe_select(), for FDD, always returns SF_DL
#ifdef Rel10
