...
void phy_procedures_UE_TX(PHY_VARS_UE *phy_vars_ue,uint8_t eNB_id,uint8_t abstraction_flag,runmode_t mode,relaying_type_t r_type)
{
  ...
  if ((slot_tx%2)==0) {  //slot 0 2 4 6 8 ...  also the beginning of every subframe
    ...
    if (phy_vars_ue->UE_mode[eNB_id] != PRACH) {
      ...
      */
      // get harq_pid from subframe relationship
      harq_pid = subframe2harq_pid(&phy_vars_ue->lte_frame_parms,  //harq_pid = (frame*2+subframe)&111
                                   frame_tx,
                                   subframe_tx);
                         ...
                         relaying_type_t r_type,PHY_VARS_RN *phy_vars_rn)
{
  ...
  for (l=0; l<n_symb; l++) {
...
#ifdef DLSCH_THREAD  //not defined
    ...
    if (((slot_rx%2)==0) && (l==0)) {  // at the beginning of every subframe and the first symbol
      // Regular PDSCH
